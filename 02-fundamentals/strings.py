...
# 1
datum = '12. 10. 2020'
print('{}-{}-{}'.format(datum[7:], datum[4:6], datum[:2]))


# 2
def text_conv(text, forma):
    print(unidecode(text.lower().replace(' ', '_')) if forma == '1' else unidecode(text.title().replace(' ', '')))


text = input('Text: ')
forma = input('Forma 1/2: ')
text_conv(text, forma)
...
def passw(pocet):
    ...
    for _ in range(pocet):
        heslo = ''.join(random.choices(abc, k=3)).upper()
        heslo += ''.join(random.choices(abc, k=3))
        heslo += ''.join(random.choice(spec))
        heslo += ''.join(random.choices(num, k=3))
        print(heslo)


pocet = input('Počet osob: ')
pocet = int(pocet)
passw(pocet)
