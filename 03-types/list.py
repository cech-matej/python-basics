...
hundreds = [num for num in range(1, 2001) if not num % 200]
print(hundreds)
...
persons += ('Anna', 32, 'žena'), ('Jiřina', 68, 'žena'), ('Vladimír', 54, 'muž'), ('Petr', 21, 'muž')
print(persons)
...
women_lam = list(filter(lambda item: item[2] == 'žena', persons))
women(women_lam)

women_com = [item for item in persons if item[2] == 'žena']
women(women_com)


def ipeople(ppl):
    ppl.sort(key=lambda item: item[1], reverse=True)
    for index, x in enumerate(ppl):
        ppl[index] = f"{index};{x[0]};{x[1]};{x[2]}"
        print(ppl[index])


ipeople_lam = list(filter(lambda item: item[0].lower().count("i") > 0, persons))
ipeople(ipeople_lam)

ipeople_com = [item for item in persons if item[0].lower().count("i") > 0]
ipeople(ipeople_com)
